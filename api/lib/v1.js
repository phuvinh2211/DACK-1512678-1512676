  ...
  switch (tx.operation) {
    ...
    case 'post':
      params = PostParams.encode({
        ...tx.params,
        content: Buffer.from(PlainTextContent.encode(tx.params.content)),
      });
      operation = 3;
      break;
    ...
    case 'update_account':
      params = UpdateAccountParams.encode({
        ...tx.params,
        value:Buffer.from(Followings.encode(tx.params.value.addresses)),
      });
      operation = 4;
      break;

    case 'interactComment':
      ...
      operation = 5;
      break;
    case 'interactReaction':
      params = InteractParams.encode({
        ...tx.params,
        object: Buffer.from(tx.params.object, 'hex'),
        content: Buffer.from(ReactContent.encode(tx.params.content)),
      });
      operation = 5;
      break;
    default:
